...
# La fonction `on_ready` est ppelée lorsque le bot est connecté à Discord
@bot.event
async def on_ready():
...
async def daily_birthday_check():
    ...
    print(f"Vérification des anniversaires pour la date : {today}", flush=True)

    # Section 1: Célébration des anniversaires d'aujourd'hui
    # -----------------------------------------------------------------
    birthdays_to_celebrate = await db.get_birthdays_today(today)
...
def get_seconds_until_midnight():
    # Ligne de test : retourne 10 secondes pour simuler un court délai
    # return 20.0

    # Ligne originale (décommenter pour l'utilisation en production)
    now = datetime.now()
    tomorrow = now + timedelta(days=1)
    midnight_tomorrow = datetime(year=tomorrow.year, month=tomorrow.month, day=tomorrow.day, hour=0, minute=0, second=0)
    delta = midnight_tomorrow - now
...
# Une fois le bot prêt, attend la bonne heure puis démarre la boucle.
...
async def before_daily_birthday_check():
    ...
    print(f"En attente de {seconds_until_midnight} secondes avant de démarrer la vérification quotidienne.", flush=True)
